...
import '../../../css/ModalTask/Proyecto/Actualizar.css';
...
export function Actualizar ({proyectoSeleccioando, changeIsShown}) {
  ...
  return (
    proyectoFirebase.map((proyectoFB) => (
      <article className='article-dialog__actualizar' key={proyectoFB.id}>
        ...
        <div className='dialog-proyecto__content'>
          ...
          <span className={`content-span__dynamic_span ${span}`}>{proyectoFB.descripcionProyecto}</span>
        ...
        <div className='dialog-button'>
            <button className='pane-content-submit__delete' type='submit' onClick={updateDescripcion}>Actualizar</button>
        </div>
      </article>
    ))
  )
